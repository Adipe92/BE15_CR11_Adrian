...
if (isset($_POST['btn-login'])) {
    ...
    if (!$error) {
        ...
        $sql = "SELECT id, first_name, pass, status FROM users WHERE email = '$email'";
...
<body>
      ...
      <div class="mt-5">
        <form class="w-100" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off">
            ...
            <input type="email" autocomplete="off" name="email" class="form-control mb-3" placeholder="Email" value="<?php echo $email; ?>" maxlength="40" />
            ...
            <hr />
            <a href="register.php">To register please click here.</a>
        </form>
    </div>
    </div>
  </div>
